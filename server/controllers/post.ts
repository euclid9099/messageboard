...
const deletePost = async ({
  ...
  await responseSkeleton(response, async () => {
    ...
    if (
      !account.admin &&
      (!post.author_id || json.payload.ID != post.author_id)
    ) {
...
const relateOnTable = async ({
  ...
  await responseSkeleton(response, async () => {
    ...
    const post = (
      await Surreal.Instance.query(`SELECT id, author_id FROM ${params.id}`)
    )[0].result[0];
    if (!post) {
      throw new Error("post doesn't exist");
    }
    ...
    if (!request.url.searchParams.has("reset")) {
      await db.query(
        `DELETE ${
          table == "likes" ? "dislikes" : "likes"
        } WHERE in = ($auth.id) AND out = $post`,
        { post: post.id }
      );
    }
